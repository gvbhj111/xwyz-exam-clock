...
const VoiceReminder = {
  ...
  playReminder(type) {
    ...
    // 未开启该提醒项则不播放
    if (!this.settings || this.settings[type] !== true) {
      console.log(`[VoiceReminder] 提醒 \`${type}\` 未开启，跳过播放`);
      return;
    }

    // 白名单强制模式：不在白名单不播放
    const currentType = subject && subject.current;
    if (this.enforceWhitelist && !this.isWhitelisted(currentType)) {
      console.log('[VoiceReminder] 当前考试类型不在白名单，已阻止语音提醒');
      return;
    }
...
// 一次性用户交互解锁音频（解决托管环境自动播放限制）
document.body.addEventListener('click', function() {
  try { EC.toggleAudio(true); } catch (e) { console.warn('[VoiceReminder] 尝试激活音频失败:', e); }
}, { once: true });
...
try {
  ...
  if (typeof EC !== 'undefined' && typeof EC.toggleAudio === 'function' && !EC.__wrappedToggleAudio) {
    ...
    EC.toggleAudio = function(state) {
      console.log(`[Audio] 切换音频系统: ${state ? '启用' : '禁用'}`);
      try {
        const res = EC.__originalToggleAudio(state);
        console.log('[Audio] 音频系统切换已执行');
        return res;
      } catch (e) {
        console.error('[Audio] 音频系统切换失败:', e);
      }
    };
    EC.__wrappedToggleAudio = true;
  }
} catch (e) {
  console.warn('[Audio] 无法包装音频方法:', e);
}
